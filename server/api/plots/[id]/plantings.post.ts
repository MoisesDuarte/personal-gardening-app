...
export default defineEventHandler(async (event) => {
  ...
  let planting
  try {
    [planting] = await db.insert(plantings).values({ plotId, cropTypeId: crop.id, plantedAt: body.data.plantedAt, expectedHarvestAt: expectedHarvestAt(body.data.plantedAt, crop) }).returning()
  } catch (cause: any) {
